...
I centralised configuration parameters into python scripts to allow the user to manage different aspects of the ETL pipeline.  The configuration options are as a result of both initial design and discovery during development.  

For example, each time I set an option during prototyping, I considered whether they should be config parameters - weighing up development cost versus value (in a proof-of-concept scope).  In general, I aimed for an *automatic* and *dynamic
...
A YAML file containing configuration options can be viewed in the [Appendix](appendix-config.qmd).

General settings include being able to filter which data to extract by way of a list of award codes.  Folders and filepaths (local and Google Drive) are created dynamically, although there are override options.
...
Additional data sources can be added to the graph database - that is, timetabling data can be augmented within non-timetabling data.  In the proof-of-concept, I added additional location data by merging with the master data source to get latitude, longitude, square meterage, etc.


### Logging

Each module has its own log file which can be expanded or contracted, as ndeeded.
