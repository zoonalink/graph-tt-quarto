...
-   `MATCH` is used to find the nodes to connect
-   `WHERE` is used to filter the nodes
-   `CREATE` is used to create the relationship
-   `...` represents additional properties
-   `MATCH` and `WHERE` are optional and can be used to find and filter nodes before creating the relationship
...
The following examples were applied to the proof-of-concept graph database to create relationships after loading data into the graph.
