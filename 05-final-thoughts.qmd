...
Universities hold a significant amount of interconnected data that can be leveraged to improve the student experience, make more informed decisions, and drive positive change.  The possibilities are endless[^1] - some blue-skies thinking is included in this [appendix page](appendix-blue-sky.qmd).

[^]: Within graph, I did not explore graph data science algorithms such as path finding, community detection, centrality, prediction, similarity, etc. There is also scope for exploring machine learning capabilities in timetabling production or optimisation.
