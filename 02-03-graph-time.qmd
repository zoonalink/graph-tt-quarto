...
The biggest challenge I encountered when translating timetables into graph data structures involved temporal elements - that is, start and end times, dates, weeks, reccurences, durations, etc. Whether a data element is a node or a property of a node, or perhaps both is relatively trivial to model and test, and depends heavily on use cases.

Time, on the other hand, is complicated, especially when you want to write aggregation queries on time elements e.g. total taught hours on campus on a Wedneday at 11:00 for student X across the whole year.
...
The basic model create nodes for each activity as they exist in the relational database. This simple approach is perfectly acceptable, but makes any time based calculations difficult because each activity node can represent a different number of occurrences because of the week ranges.

This in turn means you cannot simple COUNT each activity "equally" - for example, row one above has 11 instances, each of two hours. The second and third rows have four instances, each of three hours. Calculating aggregations, finding clashes and similar is very challenging.
...
-   **Time-Based Queries**: Answering questions about time patterns or conflicts requires traversing numerous nodes and relationships. Some queries will benefit - e.g. identifying clashes which may only occur in a specicific week, others will become more complex as denormalised data needs to be re-aggregated.
...
-   4 Timeblock nodes (09:00-09:30, etc.) - shared by ALL activities on those times!
    -   **Additional Relationships**
        ...
        -   `Activity` -\[:TAKES_PLACE_DURING\]-\> `timeBlock 09:30-10:00` (11 relationships) ... \`\`\`\
            (Activity {Name: "ITGD"}) -\[:SCHEDULED_ON\]-\> (Date {date: "2024-01-03"}) -\[:OCCUPIES\]-\> (TimeBlock {start: "09:00", end: "09:30"}) -\[:OCCUPIES\]-\> (TimeBlock {start: "09:30", end: "10:00"})

(Activity {Name: "ITGD"}) -\[:SCHEDULED_ON\]-\> (Date {date: "2024-01-03"}) -\[:TAKES_PLACE_DURING\]-\> (Timeblock {time: "09:00-09:30"}) -\[:TAKES_PLACE_DURING\]-\> (Timeblock {time: "09:30-10:00"}) -\[:TAKES_PLACE_DURING\]-\> (Timeblock {time: "10:00-10:30"}) -\[:TAKES_PLACE_DURING\]-\> (Timeblock {time: "10:30-11:00"}) ...
